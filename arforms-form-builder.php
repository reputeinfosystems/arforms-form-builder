...
$arfliteversion        = '1.7.7';
$arflitedbversion      = '1.7.7';
...
add_action( 'init', 'arforms_lite_custom_css_arr' );

function arforms_lite_custom_css_arr(){
	global $arflite_custom_css_array;
...
global $arf_api_url, $arformsmain;

$arf_api_url = $arformsmain->arfgetapiurl();
$arf_plugin_slug = basename(dirname(__FILE__));

add_filter('pre_set_site_transient_update_plugins', 'arforms_check_for_plugin_update');

function arforms_check_for_plugin_update( $checked_data ){
	$arf_lite_force_update_check = get_transient('arf_lite_force_update_check');

	if( !empty( $arf_lite_force_update_check ) ){
		global $arf_plugin_slug, $arformsmain, $arf_api_url, $wp_version, $arflitedbversion;
		...
		$args = array(
            'slug' => $arf_plugin_slug,
            'version' => $arflitedbversion,
            'other_variables' => $arformsmain->arforms_get_remote_post_params()
        );
		...
		if( isset( $response['access_request'] ) && !empty( $response['access_request'] ) && 'success' == $response['access_request'] ){
            if( isset( $response['access_package'] ) && !empty( $response['access_package'] ) ){
                ...
                if( isset( $update_package ) && is_object( $update_package ) && is_object( $checked_data ) && !empty( $update_package ) ){
                    $checked_data->response[$arf_plugin_slug .'/' . $arf_plugin_slug .'.php'] = $update_package;
                }
            }
        }
	}
...
add_filter( 'plugins_api', 'arflite_plugin_api_call', 11, 3 );

function arflite_plugin_api_call( $res, $action, $args ){
	$arf_lite_force_update_check = get_transient( 'arf_lite_force_update_check' );

	if( !empty( $arf_lite_force_update_check ) ){
		global $arf_plugin_slug, $arf_api_url, $wp_version, $arflitedbversion;

		if ( !isset( $args->slug ) || ( $args->slug != $arf_plugin_slug ) ){
            return $res;
        }

		$args->version = $arflitedbversion;
		...
		$request = wp_remote_post($arf_api_url, $request_string);

		if (is_wp_error($request)) {
			$res = new WP_Error('plugins_api_failed', '<p>'.esc_html__('An Unexpected HTTP Error occurred during the API request.', 'arforms-form-builder') . "</p> <p><a href='?' onclick='document.location.reload(); return false;>".esc_html__('Try again','arforms-form-builder')."</a></p>", $request->get_error_message());
		} else {
			$res = unserialize($request['body']);

			if ($res === false)
				$res = new WP_Error('plugins_api_failed', esc_html__('An unknown error occurred', 'arforms-form-builder'), $request['body']);
		}
	}
...
add_action( 'init', 'arf_lite_validate_plugin_update' );

function arf_lite_validate_plugin_update(){
	$arf_lite_force_update_check = get_transient( 'arf_lite_force_update_check' );
