...
class arforms_import_export_settings{
    ...
    function arforms_import_form_data(){
        ...
        if( empty( $_REQUEST['arf_import_form_nonce'] ) || ( isset(  $_REQUEST['arf_import_form_nonce'] ) && !wp_verify_nonce( $_REQUEST['arf_import_form_nonce'], 'arf_import_form' ) ) ){ //phpcs:ignore ?>
                        ...
                        foreach ( $val_main->fields->children() as $key_fields => $val_fields ) {
                            ...
                            foreach ( $val_fields as $key_field => $val_field ) {
                                ...
                                if ( $key_field == 'form_id' ) {
                                    ...
                                    $fields_option[ $key_field ] = wp_json_encode( $val_field_radio );
                                } else {

                                    if ( $key_field == 'field_options' ) {

                                        $fields_option[ $key_field ] = trim( wp_json_encode( arflite_json_decode( trim( $val_field ), true ) ) );
                                ...
                                if ( isset( $fields_option['field_options'] ) ) {
                                    ...
                                    $fields_option['field_options'] = trim( wp_json_encode( $all_field_data ) );
                                ...
                                if ( $key_field == 'field_options' ) {
                                    ...
                                    $fields_option[ $key_field ] = trim( wp_json_encode( $arf_field_options ) );
                                }
                            }
    ...
    function arflite_export_form_data() {
        ...
        if ( empty( $_POST['s_action'] ) || ( isset( $_POST['s_action'] ) && ! in_array( sanitize_text_field( $_POST['s_action'] ), array( 'arf_opt_export_form', 'arf_opt_export_both' ) ) ) ) { //phpcs:ignore
            return false;
        }
        ...
        if(isset($_POST['_wpnonce_arforms'] ) && '' == $_POST['_wpnonce_arforms'] && !wp_verify_nonce($_POST['_wpnonce_arforms'], 'arforms_wp_nonce'))
        {
            $arf_error_msg = __( 'Sorry, your request could not be processed due to security reason.', 'arforms-form-builder' );
            wp_die($arf_error_msg);
        }
        ...
        if ( isset( $_REQUEST['export_button'] ) ) {
            ...
            if ( empty( $_REQUEST['_wpnonce_arforms'] ) || (isset( $_REQUEST['_wpnonce_arforms'] ) && '' != $_REQUEST['_wpnonce_arforms'] && ! wp_verify_nonce( sanitize_text_field( $_REQUEST['_wpnonce_arforms'] ), 'arforms_wp_nonce' )) ) {
                return false;
            }
                ...
                foreach ( $res as $key => $result_array ) {
                    ...
                    foreach ( $result_array as $key => $value ) {
                        ...
                        if ( $key == 'options' ) {
                            ...
                            $value1 = wp_json_encode( $new_field );
                            ...
                            $new_form_css_val = wp_json_encode( $form_css_arry );
                    ...
                    foreach ( $res_fields as $key_fields => $result_field_array ) {
                        ...
                        foreach ( $result_field_array as $key_field => $value_field ) {
                            if ( $key_field == 'field_options' ) {
                                ...
                                $value_field_ser = wp_json_encode( $new_field1 );
                                ...
                                if ( $key_field == 'description' || $key_field == 'name' || $key_field == 'default_value' ) {
                                    ...
                                    $vl1 = '<![CDATA[' . trim( wp_json_encode( $value_field ), '"' ) . ']]>';
                                } elseif ( $key_field == 'options' ) {
                                    $vl1 = '<![CDATA[' . wp_json_encode( $value_field ) . ']]>';
                                } else {
                                    $vl1 = $value_field;
                                }
