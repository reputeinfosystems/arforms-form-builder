...
class arfdebuglog{
	...
	function __construct() {
        ...
        add_action( 'admin_enqueue_scripts', array( $this, 'arforms_load_scripts') );

        $tbl_arforms_debug_log_setting    = $wpdb->prefix . 'arf_debug_log_setting';
	}
    function arforms_load_scripts(){
    ...
    function arforms_debug_logs_func($arf_debug_log_type = '', $arf_debug_log_event = '', $arf_debug_log_event_from = '', $arf_debug_log_raw_data = '', $arf_debug_log_ref_id = 0 ){
        ...
        if ($arf_active_gateway == 1) {
            ...
            $arf_database_log_data = array(
                ...
                'arf_debug_log_raw_data'   => ( is_array( $arf_debug_log_raw_data ) ? wp_json_encode( $arf_debug_log_raw_data ) : stripslashes_deep( $arf_debug_log_raw_data ) ),
    ...
    function arforms_save_debug_log_settings_details(){
        ...
        if( 1 == $arf_is_paypal_enable && !is_plugin_active( 'arformspaypal/arformspaypal.php') ){
            global $arf_paypal;
            $arf_paypal->arf_paypal_save_debug_log_settings( $posted_data );
        }
    ...
    function arforms_view_debug_log_func(){
        ...
        if ( empty( $_POST['arflite_view_debug_log_nonce'] ) || ( isset( $_POST['arflite_view_debug_log_nonce'] ) && '' != $_POST['arflite_view_debug_log_nonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['arflite_view_debug_log_nonce'] ), 'arf_wp_nonce' ) ) ) {
                        ...
                        <tr>
                            ...
                            <td class="table-data">
                                ...
                                if( is_array($raw_data) ){
                                    ...
                                    echo wp_json_encode( $param_data );
                                }
                                else{
    ...
    function arforms_download_debug_log_func_file()
    {
        ...
        if ( empty( $_POST['_wpnone'] ) || ( isset( $_POST['_wpnone'] ) && '' != $_POST['_wpnone'] && ! wp_verify_nonce( sanitize_text_field( $_POST['_wpnone'] ), 'arf_wp_nonce' ) ) ) {
            $response['variant'] = 'error';
            $response['title'] = esc_html__( 'Error', 'arforms-form-builder');
            $response['msg'] = esc_html__( 'Sorry, your request could not be processed due to security reason.', 'arforms-form-builder' );
        ...
        if(!empty($arf_download_log_selector) && !empty($arf_download_log_duration))
        {
            ...
            $arf_download_data = wp_json_encode(stripslashes_deep($arf_download_debug_log_data));
    ...
    function arforms_clear_debug_log_data_func(){
        ...
        if( empty( $_POST['arflite_clear_debug_nonce'] ) ){
            $response['variant'] = 'error';
            $response['title'] = esc_html__( 'Error', 'arforms-form-builder');
            $response['msg'] = esc_html__( 'Sorry, your request could not be processed due to security reason.', 'arforms-form-builder' );
