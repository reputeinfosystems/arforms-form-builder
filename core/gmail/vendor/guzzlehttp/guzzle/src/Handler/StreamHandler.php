...
namespace Arforms\GuzzleHttp\Handler;

use Arforms\GuzzleHttp\Exception\ConnectException;
use Arforms\GuzzleHttp\Exception\RequestException;
use Arforms\GuzzleHttp\Promise as P;
use Arforms\GuzzleHttp\Promise\FulfilledPromise;
use Arforms\GuzzleHttp\Promise\PromiseInterface;
use Arforms\GuzzleHttp\Psr7;
use Arforms\GuzzleHttp\TransferStats;
use Arforms\GuzzleHttp\TransportSharing;
use Arforms\GuzzleHttp\Utils;
...
class StreamHandler
{
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options): void
    {
        if (\array_key_exists('curl', $options) && $options['curl'] !== null && $options['curl'] !== [] && !self::isCurlOptionGeneratedByAuth($options)) {
            \Arforms\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }
        if (\array_key_exists('expect', $options) && $options['expect'] !== \false && $request->hasHeader('Expect')) {
            \Arforms\trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
    private static function triggerConflictingStreamContextOptionDeprecations(array $streamContext): void
    {
        ...
        foreach ($streamContext as $wrapper => $contextOptions) {
            ...
            foreach ($contextOptions as $option => $_) {
                ...
                \Arforms\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing stream_context.%s.%s in the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $wrapper, $option, $conflictingOptions[$wrapper][$option]));
            }
        }
    }
    private static function triggerUnsupportedStreamContextOptionDeprecations(array $streamContext): void
    {
        ...
        \Arforms\trigger_deprecation('guzzlehttp/guzzle', '7.12', \sprintf('Passing PHP stream context options outside the built-in stream handler allow-list to the "stream_context" request option is deprecated; guzzlehttp/guzzle 8.0 will reject stream context options outside the allow-list. Deprecated option%s: %s.', \count($unsupportedOptions) === 1 ? '' : 's', \implode(', ', $unsupportedOptions)));
    }
    /**
     * @return string[]
     */
