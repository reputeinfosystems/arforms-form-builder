...
class arflitefieldcontroller {
	...
	function arflite_add_field_class( $class, $field ) {
		...
		if ( $field['type'] == 'date' ) {
			$class .= 'frm_date arforms_datepicker_control';
		}
	...
	function arflite_prevalidateform_outside() {
		...
		if ( empty( $_POST['token'] ) || ( isset( $_POST['token'] ) && '' != $_POST['token'] && ! wp_verify_nonce( sanitize_text_field( $_POST['token'] ), 'arflite_validate_outside_nonce' ) ) ) {
			echo 0;
			die;
		}
	...
	function arflite_resetformoutside() {
		...
		if ( empty( $_POST['token'] ) || ( isset( $_POST['token'] ) && '' != $_POST['token'] && ! wp_verify_nonce( sanitize_text_field( $_POST['token'] ), 'arflite_reset_form_outside_nonce' ) ) ) {
			$returnarr                = array();
	...
	function arflite_save_new_preset_field_function() {
		...
		if ( $fn != '' ) {
            ...
            if( 'csv' != strtolower( trim( $file_ext ) ) ){
                echo esc_attr( 'Not Allow This Extention File' );
                die;
            }
		...
		if ( ! empty( $arf_preset_values ) && is_array( $arf_preset_values ) ) {
			foreach ( $arf_preset_values as $key => $value ) {
				if ( '["csv_preset_' . $key . '"]' == $dynamic_field_key ) {
					...
					echo wp_json_encode( $dynamic_field_data );
				}
			}
		}
