...
class arflitemaincontroller {
	...
	function __construct() {
		...
		add_action('wp_ajax_arf_fix_broken_html', array($this, 'arf_fix_broken_html_callback'));
	}
                                ...
                                <div class="arf_whatsnew_popup_inner_content">
                                    ...
                                    You can always refer our online documentation for all the features <a href="https://www.arformsplugin.com/documentation/1-getting-started-with-arforms/" target="_blank">here</a><br>
                                        <ul style="list-style-type: disc;">
											<li>Added facility to fix broken HTML code for the HTML field</li>
    ...
    public function arf_fix_broken_html_callback(){
        ...
        $kses_processed_wrapper_html = wp_kses( $raw_html_from_post, $arf_allowed_html_tags );

        $final_html_content_for_wrapper = '';

        if (!empty($kses_processed_wrapper_html) && class_exists('DOMDocument')) {
            $temp_dom = new DOMDocument();
            libxml_use_internal_errors(true);
            $temp_dom->loadHTML('<?xml encoding="UTF-8">' . $kses_processed_wrapper_html, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
            libxml_clear_errors();

            $xpath = new DOMXPath($temp_dom);
            $wrapper_node_list = $xpath->query('//div[@class="arf_content_to_fix"]');

            if ($wrapper_node_list && $wrapper_node_list->length > 0) {
                $original_wrapper_node = $wrapper_node_list->item(0);
                
                $inner_html_to_fix = '';
                foreach ($original_wrapper_node->childNodes as $child) {
                    $inner_html_to_fix .= $temp_dom->saveHTML($child);
                }
                
                if (!empty(trim($inner_html_to_fix))) {
                    $content_dom = new DOMDocument();
                    libxml_use_internal_errors(true);
                    $content_dom->loadHTML('<?xml encoding="UTF-8"><body>' . $inner_html_to_fix . '</body>');
                    libxml_clear_errors();

                    $body_node = $content_dom->getElementsByTagName('body')->item(0);
                    if ($body_node) {
                        $fixed_inner_html = '';
                        foreach ($body_node->childNodes as $child_node) {
                            $fixed_inner_html .= $content_dom->saveHTML($child_node);
                        }
                        $final_html_content_for_wrapper = $fixed_inner_html;
                    } else {
                        $final_html_content_for_wrapper = $inner_html_to_fix; 
                    }
                } else {
                    $final_html_content_for_wrapper = '';
                }
            } else {
                if (!empty(trim($kses_processed_wrapper_html))) {
                    $content_dom = new DOMDocument();
                    libxml_use_internal_errors(true);
                    $content_dom->loadHTML('<?xml encoding="UTF-8"><body>' . $kses_processed_wrapper_html . '</body>');
                    libxml_clear_errors();
                    $body_node = $content_dom->getElementsByTagName('body')->item(0);
                    if ($body_node) {
                        $fixed_inner_html = '';
                        ...
                        $final_html_content_for_wrapper = $fixed_inner_html;
                         ...
                         $final_html_content_for_wrapper = $kses_processed_wrapper_html; // Fallback
                    }
                } else {
                     $final_html_content_for_wrapper = '';
                }
            ...
            if (preg_match('/<div class="arf_content_to_fix">(.*)<\/div>/s', $kses_processed_wrapper_html, $matches)) {
                $final_html_content_for_wrapper = $matches[1];
            } else {
                $final_html_content_for_wrapper = $kses_processed_wrapper_html;
            }
        }

        $final_html_output = '<div class="arf_content_to_fix">' . $final_html_content_for_wrapper . '</div>';
        
        wp_send_json_success(array('data' => $final_html_output));
    }
}
