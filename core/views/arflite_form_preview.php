...
if ( ! function_exists( 'arflite_display_form_preview' ) ) {
	...
	function arflite_display_form_preview( $form_id, $form_key, $posted_data = array() ) {
		...
		$options = arflite_json_decode( wp_json_encode( $posted_data->options ), true );

		$form_css = arflite_json_decode( wp_json_encode( $posted_data->form_css ), true );
		...
		$options['arf_field_order'] = wp_json_encode( $options['arf_field_order'] );

		$options['arf_field_resize_width'] = wp_json_encode( $options['arf_field_resize_width'] );
		...
		if ( isset( $posted_data->fields ) ) {
			$fields = arflite_json_decode( wp_json_encode( $posted_data->fields ) );
		}
		...
		if ( is_array( $fields ) ) {
			foreach ( $fields as $key => $value ) {
				$value->field_options = arflite_json_decode( wp_json_encode( $value->field_options ), true );
		...
		if ( ! $form->is_template && $form->id != '' ) {
			...
			$arf_form .= '<p class="arfsubmitbutton ' . esc_attr( $arf_label_pos ) . '_container">';
			$submit    = apply_filters( 'arflitegetsubmitbutton', $submit, $form );
