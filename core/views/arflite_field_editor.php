	...
	?>
		<div class="sortable_inner_wrapper edit_field_type_<?php echo esc_attr( $display['type'] ); ?>" id="arfmainfieldid_<?php echo esc_attr( $field['id'] ); ?>" inner_class="<?php echo isset( $field['inner_class'] ) ? esc_attr( $field['inner_class'] ) : 'arf_1col'; ?>" <?php echo $sortable_inner_field_style; //phpcs:ignore ?>>
			<div id="arf_field_<?php echo esc_attr( $field['id'] ); ?>" class="arfformfield control-group arfmainformfield   <?php echo isset( $newarr['position'] ) ? esc_attr( $newarr['position'] ) . '_container' : 'top_container'; ?> <?php echo ( isset( $newarr['hide_labels'] ) && $newarr['hide_labels'] == 1 ) ? 'none_container' : ''; ?> arf_field_<?php echo esc_attr( $field['id'] ); ?> ">
					...
					<div class="fieldname">
						...
						<label class="arf_main_label <?php echo esc_attr( $arf_main_label_cls ); ?>" id="field_<?php echo esc_attr( $field['id'] ); ?>">
							<span class="arfeditorfieldopt_label arf_edit_in_place">
								<input type="text" class="arf_edit_in_place_input inplace_field" onclick="arf_open_respective_style_option('arf_label')" data-ajax="false" data-field-opt-change="true" data-field-opt-key='name' value="<?php echo esc_attr( htmlspecialchars( $field['name'] ) ); ?>" data-field-id="<?php echo esc_attr( $field['id'] ); ?>" />
							</span>
				...
				<?php } ?>
						...
						case 'arfslider':
							...
							if ( $field['arf_range_selector'] == 1 ) {
								if ( is_array( $slider_value ) && count( $slider_value ) > 0 ) {
									$slider_value = wp_json_encode( array( $slider_value[0], $slider_value[1] ) );
								} else {
									$slider_value = wp_json_encode( array( $field['arf_range_minnum'], $field['arf_range_maxnum'] ) );
								}
							}
					...
					if ( $field['type'] == 'phone' && isset( $field['phonetype'] ) && $field['phonetype'] == 1 ) {
						...
						if ( isset( $phtypes ) && count( $phtypes ) > 0 ) {
							echo "<input type='hidden' id='field_" . esc_attr( $field['key'] ) . "_country_list' value='" . wp_json_encode( $phtypes ) . "' />";
				...
				$field_data_obj_array = wp_json_encode( $field_data_obj_array );
				...
				?>
				<input type="hidden" name="arf_field_data_<?php echo esc_attr( $field['id'] ); ?>" id="arf_field_data_<?php echo esc_attr( $field['id'] ); ?>" class="arf_field_data_hidden" value="<?php echo esc_attr( htmlspecialchars( wp_json_encode( $new_field_obj ) ) ); ?>" data-field_options='<?php echo wp_json_encode( $field_order ); ?>' />
